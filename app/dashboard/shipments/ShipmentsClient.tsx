/**
 * Shipments Client — ülke-bazlı sevkiyat yönetimi.
 *
 * Sevkiyat = ülke konteyner; içindeki item'lar farklı destinasyonlara gider
 * (US gemisi içinde US FBA + NJ Depo + CG Depo karma).
 *
 * Sayfa yapısı:
 *   - Üst tab: ülke (US/UK/EU/CA/AU/ZA)
 *   - "Bekleyen Havuzlar" kartı: o ülkenin her destinasyonu için
 *     COMPLETED PR (henüz sevkiyata atanmamış) sayısı, miktar
 *   - "Aktif Sevkiyatlar" listesi: o ülkenin sevkiyat kartları,
 *     her kartta destinasyon dağılımı (FBA: 80 / NJ: 40 / CG: 20)
 */
...
export interface ShipmentDTO {
  id: string;
  name: string;
  destinationTab: string; // artık ülke kodu (US/UK/EU/CA/AU/ZA)
  ...
  notes: string | null;
  stats: ShipmentStats;
  destBreakdown: Record<string, number>; // item-level destinasyon dağılımı
}

interface PoolSummary {
  destination: string;
  label: string;
  totalCompletedPRs: number;
...
export function ShipmentsClient({ activeCountry, initialShipments, canCreate }: Props) {
  ...
  // Havuz özetlerini çek (real-time)
  useEffect(() => {
    setPoolsLoading(true);
    fetch(`/api/shipments/pools?country=${activeCountry}`)
      .then(r => r.json())
      .then(d => { if (d.success) setPools(d.data.pools ?? []); })
      .catch(() => {})
      .finally(() => setPoolsLoading(false));
  }, [activeCountry]);

  const switchCountry = (country: ShipmentCountry) => {
    startTransition(() => router.replace(`/dashboard/shipments?tab=${country}`));
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/shipments', {
        ...
        body: JSON.stringify({
          name: form.name, destinationTab: activeCountry,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Gemi ve TIR yönetimi — ülke konteyner, item-level destinasyon</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {poolsLoading && <Loader2 className="w-4 h-4 animate-spin text-slate-400" />}
        </div>
        {pools.length === 0 && !poolsLoading && (
          <p className="text-xs text-slate-400">Bu ülke için bekleyen ürün yok.</p>
        )}
        {pools.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {pools.map(p => {
              const style = SHIPMENT_DESTINATION_STYLES[p.destination]
              ...
              return (
                <div key={p.destination}
                  ...
                  <div className={`text-xs font-semibold uppercase ${style.text}`}>{p.label}</div>
                  <div className="mt-1 flex items-baseline gap-2">
                    <span className={`text-xl font-bold ${style.text}`}>{p.pendingPRs}</span>
                    <span className="text-[10px] text-slate-500">PR bekliyor</span>
                  </div>
                  <div className="text-[10px] text-slate-500 mt-0.5">
                    {p.pendingQty.toLocaleString('tr-TR')} adet
                    {p.totalCompletedPRs > p.pendingPRs && (
                      <span className="ml-1 text-slate-400">
                        / {p.totalCompletedPRs} toplam tamamlanmış
                      </span>
                    )}
                  </div>
                </div>
      ...
      {canCreate && (
        <div className="flex gap-2">
          <button onClick={() => setShowCreate(!showCreate)}
            ...
            <Plus className="w-4 h-4" /> Yeni Sevkiyat ({SHIPMENT_COUNTRY_LABELS[activeCountry]})
          </button>
        </div>
      )}
      ...
      {showCreate && canCreate && (
        <form onSubmit={handleCreate} className="bg-white border border-blue-200 rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-gray-900">Yeni Sevkiyat — {SHIPMENT_COUNTRY_LABELS[activeCountry]}</h3>
      ...
      {!isPending && initialShipments.length === 0 && (
        <div className="text-center py-16 bg-white rounded-xl border">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">{SHIPMENT_COUNTRY_LABELS[activeCountry]} hattında sevkiyat yok</h3>
          ...
          {destEntries.length > 0 && (
            <div className="flex gap-1.5 mt-2 flex-wrap">
              {destEntries.map(([dest, qty]) => {
                ...
                return (
                  <span key={dest}
                    className={`px-1.5 py-0.5 rounded border text-[10px] font-medium ${style.bg} ${style.text} ${style.border}`}>
                    {label}: {qty.toLocaleString('tr-TR')}
                  </span>
                );
              })}
            </div>
