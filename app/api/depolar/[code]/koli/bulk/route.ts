...
const RowSchema = z.object({
  ...
  marketplaceCode: z.string().trim().min(1).max(50).optional(),
...
export const POST = withRoute<{ code: string }>(
  { skipAuth: true, rateLimit: 'bulk', fallbackMessage: 'Toplu koli ekleme başarısız' },
        ...
        const result = await prisma.$transaction(async (tx) => {
          ...
          const shipmentBox = await tx.shipmentBox.create({
            data: {
              ...
              iwasku: row.iwasku,
              fnsku,
              marketplaceCode: row.marketplaceCode ?? null,
              destination,
