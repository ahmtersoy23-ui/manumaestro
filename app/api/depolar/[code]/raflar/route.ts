...
export const GET = withRoute<{ code: string }>(
  { skipAuth: true, rateLimit: 'read', fallbackMessage: 'Raf listesi alınamadı' },
  async ({ request, params }) => {
    ...
    return successResponse({ shelves: result, role: auth.shelfRole, pendingUnmatched });
  }
);
...
export const POST = withRoute<{ code: string }>(
  { skipAuth: true, rateLimit: 'write', fallbackMessage: 'Raf oluşturulamadı' },
  async ({ request, params }) => {
    const { code } = params;
    const upperCode = code.toUpperCase();
