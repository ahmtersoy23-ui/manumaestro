...
export const GET = withRoute<{ code: string; shelfId: string }>(
  { skipAuth: true, rateLimit: 'read', fallbackMessage: 'Raf detayı alınamadı' },
  async ({ request, params }) => {
    const { code, shelfId: shelfIdOrCode } = params;
...
export const PATCH = withRoute<{ code: string; shelfId: string }>(
  { skipAuth: true, rateLimit: 'write', fallbackMessage: 'Raf güncellenemedi' },
  async ({ request, params }) => {
    const { code, shelfId: shelfIdOrCode } = params;
    const upperCode = code.toUpperCase();
